...
if __name__ == "__main__":
    ...
    for data_func, data_name in data_sources:
        for logN in input_sizes_log10:
            ...
            for sketch_size in sketch_sizes:
                ...
                for run_function, sketch_name in sketch_functions:
                    ...
                    if average_error == float('inf') or actual_sketch_size > MAX_ALLOWED_SIZE_IN_BYTES:
                        print(f"skipping plot for: {data_name}, logN={logN}, sketch={sketch_name}, size={sketch_size}, actual size={actual_sketch_size}, avg error={average_error}")
                        continue
                ...
                # #fig_re.savefig(plots_dir+f"{data_name}_logN={logN}_size={sketch_size}{mergingSuffix}_nolegend.pdf", format='pdf')
                # fig_re.legend()
                # #ax_re.set_title(f"{data_name}_logN={logN}_size={sketch_size}") #TODO: to be removed (now for info which plot is which)
                # ax_re.grid(True)
                # fig_re.savefig(plots_dir+f"{data_name}_logN{logN}_size{sketch_size}{mergingSuffix}_legend.pdf", format='pdf')
            ...
            yy = 4 * 16 / xx # 16 bytes per bucket/centroid
            ax_avg.plot(xx, yy, label=f"error bound")
            ax_avg.set_yscale('log')
            ax_avg.grid(True)
            fig_avg.tight_layout()
            fig_avg.savefig(plots_dir+f"{data_name}_logN{logN}{mergingSuffix}_average_nolegend.pdf", format='pdf')
            ...
            fig_avg.savefig(plots_dir+f"{data_name}_logN{logN}{mergingSuffix}_average_legend.pdf", format='pdf')

            ax_max.plot(xx, yy, label=f"error bound")
            ax_max.set_yscale('log')
            ax_max.grid(True)
            fig_max.tight_layout()
            fig_max.savefig(plots_dir+f"{data_name}_logN{logN}{mergingSuffix}_max_nolegend.pdf", format='pdf')
            ax_max.set_xlabel('sketch size in bytes')
            ax_max.set_ylabel('maximum rank error (log scale)')
            fig_avg.tight_layout()
            ax_max.legend()
            # ax_max.set_title(f"{data_name}")
            fig_max.savefig(plots_dir+f"{data_name}_logN{logN}{mergingSuffix}_max_legend.pdf", format='pdf')
