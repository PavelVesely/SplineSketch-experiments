...
if __name__ == "__main__":
    ...
    for data_func, data_name in data_sources:
        ...
        for sketch_size in sketch_sizes:
            ...
            for run_function, sketch_name in sketch_functions:
                ...
                if average_error == float('inf'):
                    print(f"skipping plot for: {data_name}, sketch={sketch_name}, size={sketch_size}")
                    continue
            ...
            ax_re.grid(True)
            fig_re.savefig(f"plots/{data_name}_size{sketch_size}_legend.pdf", format='pdf')
        ...
        fig_avg.savefig(f"plots/{data_name}_average_nolegend.pdf", format='pdf')
        ...
        fig_avg.tight_layout()
        fig_avg.savefig(f"plots/{data_name}_average_legend.pdf", format='pdf')

        ax_max.set_yscale('log')
        ax_max.grid(True)
        fig_max.tight_layout()
        fig_max.savefig(f"plots/{data_name}_max_nolegend.pdf", format='pdf')
        ax_max.set_xlabel('sketch size in bytes')
        ax_max.set_ylabel('max rank error (log scale)')
        ax_max.legend()
        fig_max.tight_layout()
        #ax_max.set_title(f"{data_name}_max")
        fig_max.savefig(f"plots/{data_name}_max_legend.pdf", format='pdf')
        ...
        ax_utm.legend()
        fig_utm.savefig(plots_dir+f"{data_name}_update_time_legend.pdf", format='pdf')
        ...
        ax_qtm.legend()
        fig_qtm.savefig(plots_dir+f"{data_name}_query_time_legend.pdf", format='pdf')
